...
asynchronously. If generation cannot be started, it returns nil and
...
4. **Insert**: Inserts the generated message into the current buffer at point when the request completes
