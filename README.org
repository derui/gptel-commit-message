...
asynchronously. While generating, the buffer is temporarily
read-only and an in-buffer indicator is shown at point. If generation
cannot be started, it returns nil and stores the message in
...
4. **Track Progress**: Shows an in-buffer indicator while the request is running
5. **Insert**: Inserts the generated message into the current buffer at point when the request completes
