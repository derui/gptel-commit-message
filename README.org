...
point. Internally it uses gptel's streaming request support and waits
for the final result. While generating, the buffer is temporarily
read-only and an in-buffer indicator is shown at point. If generation
fails, it returns nil and stores the message in
...
5. **Insert**: Inserts the generated message into the current buffer at point
