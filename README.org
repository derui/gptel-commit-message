...
inserted at point using a temporary face and then normalized once the
request completes. If generation cannot be started, it returns nil and
...
4. **Preview**: Streams partial output into the current buffer using a temporary face
